...
export class UsersService {
  ...
  async createUser(data: CreateUserDto, user: IReqUser): Promise<IUser> {
    ...
    if(user.role === ADMIN_ROLES.ADMIN) {
      const fetchAgent = await this.agentsService.getAgentWithLeastProspect(city)
      if(!fetchAgent.agent_id) throw new NotFoundException('no agent found in this city')
  ...
  async loginUser(data: LoginDto, res: Response) {
    ...
    const user: Users = await this.usersRepository.findOne({ where: { email }})
    if (!user?.id) {
      throw new NotFoundException(`user with email: ${data.email} not found`)
    }

    const isPasswordValid = await UtilService.validatePassword(password, user?.password);
    if (!isPasswordValid) {
  ...
  async forgotPassword(data: ForgotPasswordDto) {
    ...
    if (!user?.id) {
      throw new HttpException(`user with email: ${data.email} not found`, HttpStatus.NOT_FOUND);
    }
